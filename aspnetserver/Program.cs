...
{
    swaggerGenOptions.SwaggerDoc("v1", new Microsoft.OpenApi.Models.OpenApiInfo { Title = "ASP.NET", Version = "v1" });
});
...
{ 
    ...
    swaggerUIOptions.SwaggerEndpoint(url: "/swagger/v1/swagger.json","Web API servingavery simple Post model.");
...
app.MapGet(pattern: "/get-post-by-id/{postId}", async (int postId) =>
{
...
app.MapPost("/create-post", async (Post postToCreate) =>
{
    ...
    if (createSuccessful)
    {
        return Results.Ok("Post utworzony.");
    }
    else
    {
        return Results.BadRequest();
...
app.MapPut("/update-post", async (Post postToUpdate) =>
{
    ...
    if (updateSuccessful)
    {
        return Results.Ok("Post zaktualizowany.");
    }
    else
    {
        return Results.BadRequest();
...
app.MapDelete("/delete-post-by-id/{postId}", async (int postId) =>
{
    ...
    if (deleteSuccessful)
    {
        return Results.Ok("Post usuniêty.");
    }
    else
    {
        return Results.BadRequest();
